...
<script>
import WCard from '@/components/yearsCard/vipCard'
...
export default {
  ...
  methods: {
    ...
    async profile () {
      ...
      this.userId = res.id
      this.userInfo = res
      this.userInfo.VIPExpiredAt = this.$wxApi.getTime(res.VIPExpiredAt)
    },

    // 用户信息-会员中心
    async getUserVipInfo (data) {
      let res = await this.$api.share.getUserShareDetail(data)
      if (res.msg) return
      // this.vipInfo = res
      this.user = [ ...res.activity.cardActivityRecords ]
      for (let i = 0; i < 5; i++) {
        if (!this.user[i]) {
          // 没有5个时，数组为5个状态为3的值
          let obj = {
      ...
      // 需要分享次数返现
      this.allTimes = res.activity.cardActivity.passLine
      this.times = res.activity.cardActivity.lines
      // 当前次数
      this.currentTimes = res.activity.cardActivityRecords.length
      let len = this.times - res.activity.cardActivityRecords.length
