  ...
  methods: {
    ...
    toPage (index) {
      ...
      if (index === 1) {
        ...
        wx.navigateTo({
          url: `../selectCard/main?childId=${childId}`
        })
      }
    },
    ...
    async getChild (id) {
      ...
      if (this.cardType === 4) {
        ...
        let arr = r.list.filter(i => i.remainingTimes > 0 && i.childId === 0 && i.cardType.code === 'TY1003')
        arr = arr.map(j => ({
          ...j,
          myCardCode: j.cardType && j.cardType.code
        }))
        // 有未绑定的卡
        if (arr.length) {
          // 年卡
          // 体验卡,试听卡不可用
          let exCard = arr.filter(e => e.myCardCode === 'TY1003' && e.cardType.limitTimes >= 1)
          let data = {
            cardId: exCard[0].id
          }
    ...
    async booking (e) {
      ...
      console.log('booking cardType', cardType)
      // 购买18课时
      if (cardType === 0) {
        // 购买课时
        let d = {
      ...
      if (cardType === 1) {
        // 预约课时不够 =》购买
        // 当前剩余课时 小于 课程课时
        // console.log('this. userInfo', this.userInfo, this.classItem)
        ...
        //       if (res.confirm) {
        //         wx.navigateTo({
        //           url: `../packageMe/buyLesson?type=1`
        //         })
        //         console.log('用户点击确定')
        //       } else if (res.cancel) {
        //         console.log('用户点击取消')
    ...
    async getCurrencyGoods (param) {
      ...
      console.log('this. lessonPrice', this.lessonPrice)
    },
    ...
    chooseCard () {
      ...
      console.log('cardType', this.cardType, this.buyYearCard)
    },
