<template>
  <div :class="joinStatus !== 1 ? 'msg-wrap' : ''">
    <!-- todo -->
    <!-- 打卡信息不显示该片段 -->
    <div class="msg-box-title">
      <span class="msg-title" 
        @click.stop="toTheme(item)" v-for="(item, index) in (type && type.topics)"
        :key="index"># {{ item }}</span>
    ...
    <div v-if="type.infoType === 3" class="img-content">
      <div :class="[type.detail.pics.length === 1 ? 'img-one-class' : 'img-size']"
        @click.stop="lookImg(index)"
        v-for="(item, index) in type.detail.pics"
        :key="index">
    ...
    <div v-if="type.infoType === 5" @click.stop="playVideo(1)">
      <div v-if="type.detail.thumbUrl">
        <video v-if="curr_id === type.id" :style="{ height: imgHeight + 'rpx', width: imgWidth + 'rpx' }"
          @pause="bindpause(type.id)" :id="type.id"
          @play="bindVideoPlay(type.id)"
          @ended="bindended(type.id)"
          objectFit="contain" :src="type.detail && type.detail.url"></video>
        <div v-else class="img-mask-box"
          ...
          <div class="video-time">{{ typeMsg }}s</div>
      ...
      <div v-else>
        <video objectFit="contain" :src="type.detail && type.detail.url"></video>
      </div>
    </div>
    <!-- 文件 -->
    ...
    <!-- <div v-if="type.infoType === 2" class="progress"> -->
      <!-- 有限次打卡 -->
      <!-- todo -->
    ...
    <div v-if="type.infoType === 9" class="questions">
      ...
      <!-- todo -->
      <div v-if="dataDetail.task && dataDetail.task.questionType === 2" class="content-flex">
      <!-- <div v-if="dataDetail.questionType === 20" class="content-flex"> -->
        <div  class="content-flex" v-for="(item, index) in dataDetail.answers" :key="index">
          <div :class="[ item.pics && item.pics.length === 1 ? 'img-one-class' : 'img-size' ]" v-for="(picsItem, picsIndex) in item.pics" :key="picsIndex">
            <image mode="aspectFill" @click.stop="questionLookImg(picsIndex)" :class="[ item.pics && item.pics.length === 1 ? 'img-one-class' : 'img-size' ]" :src="picsItem" />
          </div>
        </div>
      </div>
      ...
      <div v-if="dataDetail.task && dataDetail.task.questionType === 4" class="questions-box">
        <div class="questions-video" @click.stop="playVideo(1)" v-for="(item, index) in dataDetail.answers" :key="index">
          <div v-if="item.video && item.video.thumbUrl">
            <video v-if="curr_id === type.id"
              :style="{ height: imgHeight + 'rpx', width: imgWidth + 'rpx' }"
              :id="dataDetail.taskId" objectFit="contain"
            ...
            <div v-else 
              ...
              <image mode="aspectFill" @load="imageLoad" :style="{ height: imgHeight + 'rpx', width: imgWidth + 'rpx' }"
                class="video-img-mask" :src="item && item.video.thumbUrl" />
          ...
          <div v-else>
            <video objectFit="contain" :src="item.video.url"></video>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    typeMsg () {
      let time = 0
      time = Math.floor(this.type.detail && this.type.detail.duration)
      return time
    },
    lookContent () {
      let content = ''
      content = this.type.textContent
      if (this.type && this.type.textContent && this.type.textContent.length > 100) {
  ...
  methods: {
    ...
    get () {
      if (this.dataDetail && this.dataDetail.answers) {
        ...
        // 深度拷贝
        this.lookAnswers = JSON.parse(JSON.stringify(this.dataDetail.answers)).map((i) => {
